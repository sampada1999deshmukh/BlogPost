...
def register():
    ...
    if User.query.filter_by(email=form.email.data).first():
        ...
        hash_salt_pwd = generate_password_hash(form.password.data, method="pbkdf2:sha256", salt_length=8)
        new_user = User(
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash("Email is not exist in Database")
            return redirect(url_for('login'))
